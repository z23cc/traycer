...
/**
 * Every native session that already has a chat here, keyed
 * `harness:nativeSessionId` and naming that chat. A scan at @1.0 hides these;
 * from @1.1 it offers them back marked `already_in_traycer` so the wizard can
 * open the chat instead of importing it twice.
 *
 ...
 */
export function sessionsAlreadyInTraycer(
  runtime: HostRuntime,
): Map<string, { readonly epicId: string; readonly chatId: string }> {
  const chats = new Map<
    string,
  ...
  for (const chat of runtime.store.snapshot().chats) {
    ...
    if (session !== null) {
      chats.set(`${session.harnessId}:${session.sessionId}`, {
        epicId: chat.epicId,
        chatId: chat.chatId,
      });
    }
